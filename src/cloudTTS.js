...
async function say(text, voiceOptions, playOptions, callback) {
  if (voiceOptions === undefined) {voiceOptions = {
    languageCode: 'ja-JP',
    name: 'ja-JP-Wavenet-A',
    ssmlGender: 'NEUTRAL'
  }}
  if (playOptions === undefined) {playOptions = {}}
  ...
  return Promise.resolve(request)
    ...
    .then(function(response){
      return new Promise(function(resolve, reject){
        const len = 20
        const filenameHash = Math.random().toString(36).slice(-len).replace(/\./, '')
        // const filenameHash = ''
        const filename = filenamePrefix + '_' + filenameHash + '.mp3'

        const writeFile = util.promisify(fs.writeFile)
        writeFile(filename, response.audioContent, 'binary', function(){resolve(filename)})
      })
    })
