...
2. **外国語コメントの自然な処理**:
   - **カタカナ変換モード (`KATAKANA`)**: 英語・ロシア語・スペイン語・韓国語（音素分解＋連音化）・中国語（台湾標準語/スラング優先・ピンイン）などのコメントを、発音規則・音素分解・辞書に基づいて自然なカタカナに自動変換し、日本語ボイスで違和感なくスムーズに読み上げます（漢字のみの日本語コメントを誤判定しない安全ガード付き）。
