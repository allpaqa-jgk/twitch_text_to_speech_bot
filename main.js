...
function checkConfig() {
  ...
  if (fs.existsSync(path.join(__dirname, "./config/serviceAccount.json"))) {
    ...
    fs.existsSync(path.join(__dirname, "./config/service_account.json"))
  ) {
    console.info("overwrite GOOGLE_APPLICATION_CREDENTIALS!");
    process.env.GOOGLE_APPLICATION_CREDENTIALS = path.join(
      __dirname,
  ...
  if (!config.TW_OAUTH_TOKEN) {
    configWarning.push(
      "TW_OAUTH_TOKEN are not found. please set it at config file"
    );
  }
  if (!config.TW_CHANNEL_NAME) {
    configWarning.push(
      "TW_CHANNEL_NAME are not found. please set it at config file"
    );
  }
  if (
    config.TTS_MODE === "CloudTTS" &&
    !process.env.GOOGLE_APPLICATION_CREDENTIALS
  ) {
    configWarning.push(
      "GOOGLE_APPLICATION_CREDENTIALS are not found. please set it at PATH or serviceAccount.json"
    );
  }
